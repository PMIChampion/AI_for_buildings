    ...
    def forward(self, x):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x)
        # Замена rearrange на PyTorch операции
        x = x.view(x.size(0), self.input_resolution[0], self.input_resolution[1], -1).permute(0, 3, 1, 2)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            x = self.scale * x + _x
        # Замена rearrange на PyTorch операции
        x = x.view(x.size(0), self.patches_resolution[0], self.patches_resolution[1], -1).permute(0, 3, 1, 2)
        return x