...
class MANIQA(nn.Module):
    ...
    def forward(self, x):
        ...
        # stage 1
        x = x.permute(0, 2, 1)  # Замена rearrange 'b (h w) c -> b c (h w)'
        for tab in self.tablock1:
            x = tab(x)
        x = x.view(x.size(0), x.size(1), self.input_size, self.input_size)  # Замена rearrange 'b c (h w) -> b c h w'
        x = self.conv1(x)
        x = self.swintransformer1(x)

        # stage2
        x = x.view(x.size(0), x.size(1), -1)  # Замена rearrange 'b c h w -> b c (h w)'
        ...
        x = x.permute(0, 2, 3, 1).view(x.size(0), -1, x.size(1))  # Замена rearrange 'b c h w -> b (h w) c'
